...
const VideoChat = {
  ...
  onToken: function (callback) {
    ...
    return (token) => {
      ...
      VideoChat.localStream.getTracks().forEach((track) => {
...
function rePositionLocalVideo() {
  ...
  const bounds = remoteVideo.position()
  bounds.top += 10
  bounds.left += 10
...
function muteMicrophone() {
  ...
  VideoChat.peerConnection.getSenders().find((s) => {
...
function switchStreamHelper(stream) {
  ...
  if (VideoChat.connected) {
    // Find sender
    const sender = VideoChat.peerConnection.getSenders().find(
      (
        s // make sure tack types match
  ...
  VideoChat.recognition.onresult = (event) => {
    ...
    for (let i = event.resultIndex, len = event.results.length; i < len; i++) {
      ...
      if (event.results[i].isFinal) {
        ...
        const charsToKeep = interimTranscript.length % 100
